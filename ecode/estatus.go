...
import (
	...
	"github.com/gisvr/protocode/shared-go"
...
func Error(code Code, message string) *Status {
	return &Status{s: &pbshared.Status{Code: int32(code.Code()), Message: message}}
}
...
func MakePBStatus(code Code, format string, args ...interface{}) *pbshared.Status {
	return Errorf(code, format, args...).Proto()
}

func PbStatus2Status(ps *pbshared.Status) *Status {
	s := &Status{
		s: ps,
	}
	return s
